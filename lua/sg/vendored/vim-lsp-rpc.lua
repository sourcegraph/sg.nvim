...
---@return string: table containing encoded message and `Content-Length` attribute
...
---@class VendoredPublicClient
---@field notify function(method, params)
---@field request function(method, params, callback)
...
---@param callback function(err: lsp.ResponseError|nil, result: any): Callback to invoke
---@param notify_reply_callback function|nil: Callback to invoke as soon as a request is no longer pending
---@return boolean: success
---@return integer|nil: request_id true, request_id if request could be sent, `false` if not
    ...
    schedule(function()
      coroutine.wrap(function()
        ...
        if status then
          ...
          if err then
            _ = assert(type(err) == "table", "err must be a table. Use rpc_response_error to help format errors.")
        ...
        if type(result) == "function" then
          result(function(async_result)
            self:send_response(decoded.id, nil, async_result)
          end)
        else
...
---@private
---@param client VendoredRpcClient
...
local function public_client(client)
  ...
  ---@param notify_reply_callback function|nil: Callback to invoke as soon as a request is no longer pending
  ---@return boolean: success
  ---@return integer|nil: request_id true, message_id if request could be sent, `false` if not
...
local function start(cmd, cmd_args, dispatchers, extra_spawn_params)
  ...
  if extra_spawn_params.cwd then
    local _ = assert(is_dir(extra_spawn_params.cwd), "cwd must be a directory")
  end
