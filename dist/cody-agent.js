  ...
  "../../node_modules/.pnpm/@microsoft+fetch-event-source@2.0.1/node_modules/@microsoft/fetch-event-source/lib/cjs/fetch.js"(exports) {
    ...
    function fetchEventSource2(input, _a) {
      ...
      return new Promise((resolve, reject) => {
        ...
        const fetch3 = inputFetch !== null && inputFetch !== void 0 ? inputFetch : window.fetch;
...
// ../cody-shared/src/prompt/templates.ts
...
function populateCurrentEditorContextTemplate(code, filePath, repoName) {
  const context = isMarkdownFile(filePath) ? populateMarkdownContextTemplate(code, filePath, repoName) : populateCodeContextTemplate(code, filePath, repoName);
  return (repoName ? CURRENT_EDITOR_CODE_TEMPLATE_WITH_REPO.replace("{repoName}", repoName) : CURRENT_EDITOR_CODE_TEMPLATE).replace(/{filePath}/g, filePath) + context;
}
var CURRENT_EDITOR_SELECTED_CODE_TEMPLATE = "I am currently looking at this part of the code from `{filePath}`. ";
var CURRENT_EDITOR_SELECTED_CODE_TEMPLATE_WITH_REPO = "I am currently looking at this part of the code from `{filePath}` in repository {repoName}. ";
function populateCurrentEditorSelectedContextTemplate(code, filePath, repoName) {
  const context = isMarkdownFile(filePath) ? populateMarkdownContextTemplate(code, filePath, repoName) : populateCodeContextTemplate(code, filePath, repoName);
  return (repoName ? CURRENT_EDITOR_SELECTED_CODE_TEMPLATE_WITH_REPO.replace("{repoName}", repoName) : CURRENT_EDITOR_SELECTED_CODE_TEMPLATE).replace(/{filePath}/g, filePath) + context;
}
...
function getExtension(filePath) {
  return import_path.default.extname(filePath).slice(1);
}

// ../cody-shared/src/utils.ts
var isError = (value) => value instanceof Error;

// ../cody-shared/src/codebase-context/messages.ts
function getContextMessageWithResponse(text, file, response = "Ok.") {
  return [
...
// ../cody-shared/src/codebase-context/index.ts
var CodebaseContext = class {
  ...
  async getContextMessages(query, options) {
    switch (this.config.useContext) {
      ...
      case "none":
        return [];
      default:
        return this.embeddings ? this.getEmbeddingsContextMessages(query, options) : this.getLocalContextMessages(query, options);
    }
  }
  ...
  async getEmbeddingsContextMessages(query, options) {
    ...
    return groupResultsByFile(combinedResults).reverse().flatMap((groupedResults) => this.makeContextMessageWithResponse(groupedResults)).map((message) => contextMessageWithSource(message, "embeddings"));
  }
...
// ../cody-shared/src/embeddings/client.ts
var SourcegraphEmbeddingsSearchClient = class {
  constructor(client, repoId, web = false) {
    this.client = client;
    this.repoId = repoId;
    this.web = web;
  }
...
// ../cody-shared/src/intent-detector/client.ts
var editorRegexps = [/editor/, /(open|current|this)\s+file/, /current(ly)?\s+open/, /have\s+open/];
...
// ../cody-shared/src/sourcegraph-api/completions/browserClient.ts
var import_fetch_event_source = __toESM(require_cjs());

// ../cody-shared/src/sourcegraph-api/completions/client.ts
...
// ../cody-shared/src/sourcegraph-api/graphql/client.ts
var import_isomorphic_fetch = __toESM(require_fetch_npm_node());

// ../http-client/src/graphql/graphql.ts
var import_lodash = __toESM(require_lodash());

// ../http-client/src/graphql/constants.ts
...
// ../cody-shared/src/sourcegraph-api/graphql/queries.ts
...
var SourcegraphGraphQLAPIClient = class {
  constructor(config) {
    this.config = config;
    this.dotcomUrl = "https://sourcegraph.com";
  }
  onConfigurationChange(newConfig) {
    this.config = newConfig;
  }
  ...
  async getCodyLLMConfiguration() {
    const response = await this.fetchSourcegraphAPI(CURRENT_SITE_CODY_LLM_CONFIGURATION);
  ...
  async getRepoId(repoName) {
    return this.fetchSourcegraphAPI(REPOSITORY_ID_QUERY, {
      ...
      (response) => extractDataOrError(
        response,
        (data) => data.repository ? data.repository.id : new RepoNotFoundError(`repository ${repoName} not found`)
      )
    );
  }
  ...
  async logEvent(event) {
    ...
    try {
      if (this.config.serverEndpoint === this.dotcomUrl) {
        await this.fetchSourcegraphAPI(LOG_EVENT_MUTATION, event).then(
          (response) => {
        ...
        await Promise.all([
          this.fetchSourcegraphAPI(LOG_EVENT_MUTATION, event).then(
            (response) => {
              extractDataOrError(response, (data) => {
              });
            }
          ),
          this.fetchSourcegraphDotcomAPI(LOG_EVENT_MUTATION, event).then(
            (response) => {
              extractDataOrError(response, (data) => {
              });
            }
  ...
  fetchSourcegraphAPI(query, variables = {}) {
    ...
    return (0, import_isomorphic_fetch.default)(url, {
      method: "POST",
      body: JSON.stringify({ query, variables }),
      headers
    }).then(verifyResponseCode).then((response) => response.json()).catch((error) => new Error(`accessing Sourcegraph GraphQL API: ${error} (${url})`));
  }
  // make an anonymous request to the dotcom API
  async fetchSourcegraphDotcomAPI(query, variables) {
...
// ../cody-shared/src/chat/bot-response-multiplexer.ts
...
var _BotResponseMultiplexer = class {
  constructor() {
    this.subs_ = /* @__PURE__ */ new Map();
  ...
  async publish(response) {
    ...
    while (this.buffer_) {
      ...
      const match = this.buffer_.match(_BotResponseMultiplexer.TOPIC_RE);
      if (!match) {
...
var BotResponseMultiplexer = _BotResponseMultiplexer;
/**
 * The default topic. Messages without a specific topic are sent to the default
 * topic subscriber, if any.
 */
BotResponseMultiplexer.DEFAULT_TOPIC = "Assistant";
// Matches topic open or close tags
BotResponseMultiplexer.TOPIC_RE = /<$|<\/?([A-Za-z-]?$|[A-Za-z-]+>?)/m;

// ../cody-shared/src/prompt/constants.ts
...
var MAX_AVAILABLE_PROMPT_LENGTH = DEFAULT_MAX_TOKENS - ANSWER_TOKENS;
var CHARS_PER_TOKEN = 4;

// ../cody-shared/src/chat/typewriter.ts
var MAX_DELAY_MS = 200;
var MIN_DELAY_MS = 5;
var MIN_CHAR_CHUNK_SIZE = 1;
var createTypewriter = ({ emit }) => {
  let fullText = "";
  let processedText = "";
  let interval;
  return {
    write: (updatedText) => {
      ...
      const charChunkSize = calculatedDelay < MIN_DELAY_MS ? Math.round(MIN_DELAY_MS / calculatedDelay) : MIN_CHAR_CHUNK_SIZE;
...
// ../cody-shared/src/chat/preamble.ts
var actions = `You are Cody, an AI-powered coding assistant created by Sourcegraph. You work inside a text editor. You have access to my currently open files. You perform the following actions:
...
var rules = `In your responses, obey the following rules:
...
var answer = `Understood. I am Cody, an AI assistant made by Sourcegraph to help with programming tasks.
...
// ../cody-shared/src/prompt/truncation.ts
...
// ../cody-shared/src/chat/transcript/interaction.ts
var Interaction = class {
  constructor(humanMessage, assistantMessage, fullContext, usedContextFiles, timestamp = (/* @__PURE__ */ new Date()).toISOString()) {
  ...
  setUsedContext(usedContextFiles) {
    this.usedContextFiles = usedContextFiles;
  }
  /**
...
// ../cody-shared/src/chat/recipes/chat-question.ts
var ChatQuestion = class {
  constructor(debug) {
    this.debug = debug;
    this.id = "chat-question";
  }
...
// ../cody-shared/src/chat/recipes/helpers.ts
...
async function getContextMessagesFromSelection(selectedText, precedingText, followingText, { fileName, repoName, revision }, codebaseContext) {
  ...
  return selectedTextContext.concat(
    [precedingText, followingText].flatMap(
...
function getFileExtension(fileName) {
  return import_path2.default.extname(fileName).slice(1).toLowerCase();
}

// ../cody-shared/src/chat/recipes/explain-code-detailed.ts
var ExplainCodeDetailed = class {
  constructor() {
    this.id = "explain-code-detailed";
  }
...
// ../cody-shared/src/chat/recipes/explain-code-high-level.ts
var ExplainCodeHighLevel = class {
  constructor() {
    this.id = "explain-code-high-level";
  }
  async getInteraction(_humanChatInput, context) {
    const selection = context.editor.getActiveTextEditorSelectionOrEntireFile();
    if (!selection) {
...
// ../cody-shared/src/chat/recipes/find-code-smells.ts
var FindCodeSmells = class {
  constructor() {
    this.id = "find-code-smells";
  }
  async getInteraction(_humanChatInput, context) {
    const selection = context.editor.getActiveTextEditorSelectionOrEntireFile();
    if (!selection) {
...
// ../cody-shared/src/chat/recipes/generate-docstring.ts
var GenerateDocstring = class {
  constructor() {
    this.id = "generate-docstring";
  }
  async getInteraction(_humanChatInput, context) {
    const selection = context.editor.getActiveTextEditorSelectionOrEntireFile();
    if (!selection) {
...
// ../cody-shared/src/chat/recipes/generate-test.ts
var GenerateTest = class {
  constructor() {
    this.id = "generate-unit-test";
  }
  async getInteraction(_humanChatInput, context) {
    const selection = context.editor.getActiveTextEditorSelectionOrEntireFile();
    if (!selection) {
...
var ImproveVariableNames = class {
  constructor() {
    this.id = "improve-variable-names";
  }
  async getInteraction(_humanChatInput, context) {
    const selection = context.editor.getActiveTextEditorSelectionOrEntireFile();
    if (!selection) {
...
// ../cody-shared/src/chat/recipes/langs.ts
...
// ../cody-shared/src/chat/recipes/translate.ts
var TranslateToLanguage = class {
  constructor() {
    this.id = "translate-to-language";
  }
  async getInteraction(_humanChatInput, context) {
    const selection = context.editor.getActiveTextEditorSelectionOrEntireFile();
    if (!selection) {
...
TranslateToLanguage.options = languageNames;

// ../cody-shared/src/chat/recipes/browser-recipes.ts
...
function nullLog(filterLabel, text, ...args) {
}
function init() {
...
// ../cody-shared/src/prompt/prompt-mixin.ts
var PromptMixin = class {
  /**
...
// ../cody-shared/src/chat/transcript/index.ts
var Transcript = class {
  ...
  setUsedContextFilesForLastInteraction(contextFiles) {
    if (this.interactions.length === 0) {
  ...
  reset() {
    this.interactions = [];
    this.internalID = (/* @__PURE__ */ new Date()).toISOString();
  }
};
...
// ../cody-shared/src/chat/viewHelpers.ts
var STOP_SEQUENCE_REGEXP = /(H|Hu|Hum|Huma|Human|Human:)$/;
function reformatBotMessage(text, prefix) {
...
// ../cody-shared/src/chat/client.ts
async function createClient({
  ...
  editor,
  initialTranscript,
  createCompletionsClient = (config2) => new SourcegraphBrowserCompletionsClient(config2)
}) {
...
// ../cody-shared/src/chat/recipes/agent-recipes.ts
function nullLog2(filterLabel, text, ...args) {
}
var registeredRecipes2 = {};
function getRecipe2(id) {
  return registeredRecipes2[id];
}
function registerRecipe2(id, recipe) {
  registeredRecipes2[id] = recipe;
}
function init2() {
  if (Object.keys(registeredRecipes2).length > 0) {
    return;
  }
  const recipes = [
    new ChatQuestion(nullLog2),
    new ExplainCodeDetailed(),
    new ExplainCodeHighLevel(),
    new GenerateDocstring(),
    new GenerateTest(),
  ...
  for (const recipe of recipes) {
    const existingRecipe = getRecipe2(recipe.id);
    if (existingRecipe) {
      throw new Error(`Duplicate recipe with ID ${recipe.id}`);
    }
    registerRecipe2(recipe.id, recipe);
  }
}
init2();

// ../cody-shared/src/sourcegraph-api/completions/nodeClient.ts
var import_http = __toESM(require("http"));
var import_https = __toESM(require("https"));

// ../cody-shared/src/sourcegraph-api/utils.ts
...
// ../cody-shared/src/sourcegraph-api/completions/parse.ts
var EVENT_LINE_PREFIX = "event: ";
var DATA_LINE_PREFIX = "data: ";
var EVENTS_SEPARATOR = "\n\n";
...
function parseEventData(eventType, dataLine) {
  ...
  switch (eventType) {
    case "completion": {
      ...
      if (typeof data.completion === void 0) {
...
function parseEvents(eventsBuffer) {
...
// ../cody-shared/src/sourcegraph-api/completions/nodeClient.ts
var SourcegraphNodeCompletionsClient = class extends SourcegraphCompletionsClient {
  async complete(params, abortSignal) {
    ...
    try {
      const response2 = JSON.parse(result);
      if (typeof response2.completion !== "string" || typeof response2.stopReason !== "string") {
        const message = `response does not satisfy CodeCompletionResponse: ${result}`;
        log?.onError(message);
        throw new Error(message);
      } else {
        log?.onComplete(response2);
        return response2;
      }
    } catch (error) {
  ...
  stream(params, cb) {
    ...
    const requestFn = this.completionsEndpoint.startsWith("https://") ? import_https.default.request : import_http.default.request;
    const request = requestFn(
      ...
      (res) => {
        ...
        if (res.statusCode >= 400) {
          let bufferBin2 = Buffer.of();
          let errorMessage = "";
          res.on("data", (chunk) => {
            if (!(chunk instanceof Buffer)) {
              throw new TypeError("expected chunk to be a Buffer");
            }
            const { str, buf } = toPartialUtf8String(Buffer.concat([bufferBin2, chunk]));
            errorMessage += str;
            bufferBin2 = buf;
          });
          res.on("error", (e) => {
    ...
    request.on("error", (e) => {
      ...
      if (message.includes("ECONNREFUSED")) {
        message = "Could not connect to Cody. Please ensure that Cody app is running or that you are connected to the Sourcegraph server.";
      }
      log?.onError(message);
      cb.onError(message);
    });
...
var DocumentOffsets = class {
  constructor(document2) {
    ...
    if (document2.content) {
      this.lines.push(0);
      let index = 1;
      while (index < document2.content.length) {
        if (document2.content[index] === "\n") {
          this.lines.push(index + 1);
        }
        index++;
      }
      if (document2.content.length !== this.lines[this.lines.length - 1]) {
        this.lines.push(document2.content.length);
      }
    }
  }
  offset(position) {
...
var AgentEditor = class {
  ...
  getActiveTextEditor() {
    const document2 = this.activeDocument();
    if (document2 === void 0) {
      return null;
    }
    return {
  ...
  getActiveTextEditorSelection() {
    const document2 = this.activeDocument();
    if (document2 === void 0 || document2.content === void 0 || document2.selection === void 0) {
      return null;
    }
    const offsets = new DocumentOffsets(document2);
    const from = offsets.offset(document2.selection.start);
    const to = offsets.offset(document2.selection.end);
    return {
      fileName: document2.filePath || "",
      precedingText: document2.content.slice(0, from),
      selectedText: document2.content.slice(from, to),
      followingText: document2.content.slice(to, document2.content.length)
    };
  }
  getActiveTextEditorSelectionOrEntireFile() {
    const document2 = this.activeDocument();
    if (document2 !== void 0 && document2.selection === void 0) {
      return {
        fileName: document2.filePath || "",
        precedingText: "",
        selectedText: document2.content || "",
        followingText: ""
      };
    }
...
var MessageDecoder = class extends import_stream.Writable {
      ...
      while (true) {
        if (this.contentLengthRemaining === null) {
          ...
          if (this.contentLengthRemaining === 0) {
            try {
              const data2 = JSON.parse(this.contentBuffer.toString());
              this.contentBuffer = Buffer.alloc(0);
              this.contentLengthRemaining = null;
              this.callback(null, data2);
            } catch (error) {
...
var MessageEncoder = class extends import_stream.Readable {
  constructor() {
    super(...arguments);
    this.buffer = Buffer.alloc(0);
  }
  send(data) {
    ...
    const header = Buffer.from(`Content-Length: ${content.byteLength}\r
\r
`, "utf-8");
...
var MessageHandler = class {
  constructor() {
    ...
    this.messageDecoder = new MessageDecoder((err, msg) => {
      if (err) {
...
// src/agent.ts
var Agent = class extends MessageHandler {
  constructor() {
    super();
    this.workspaceRootPath = null;
    this.activeDocumentFilePath = null;
...
`
    ...
    this.registerNotification("textDocument/didFocus", (document2) => {
      this.activeDocumentFilePath = document2.filePath;
    });
    this.registerNotification("textDocument/didOpen", (document2) => {
      this.documents.set(document2.filePath, document2);
      this.activeDocumentFilePath = document2.filePath;
    });
    this.registerNotification("textDocument/didChange", (document2) => {
      if (document2.content === void 0) {
        document2.content = this.documents.get(document2.filePath)?.content;
      }
      this.documents.set(document2.filePath, document2);
      this.activeDocumentFilePath = document2.filePath;
    });
    this.registerNotification("textDocument/didClose", (document2) => {
      this.documents.delete(document2.filePath);
    });
    ...
    this.registerRequest(
      "recipes/list",
      () => Promise.resolve(
        Object.values(registeredRecipes2).map(({ id }) => ({
          id,
          title: id
          // TODO: will be added in a follow PR
        }))
      )
    );
  ...
  setClient(config) {
    this.client = createClient({
      ...
      createCompletionsClient: (config2) => new SourcegraphNodeCompletionsClient(config2)
    });
  }
};
